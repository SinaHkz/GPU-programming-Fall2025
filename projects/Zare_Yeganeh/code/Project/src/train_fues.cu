...
static float evaluate_mnist(
    ...
    for (int start = 0; start + B <= test_ds.num; start += B) {
        ...
        conv_forward_fused_bias_relu(&d_X, &d_w1, d_b1, &d_R1, pad, stride, threads);
        kcheck("conv1_fused");

        maxpool_forward(&d_R1, &d_P1, d_argmax1, pool, pool_stride, threads);
        kcheck("pool1");

        conv_forward_fused_bias_relu(&d_P1, &d_w2, d_b2, &d_R2, pad, stride, threads);
        kcheck("conv2_fused");
